...
//
// Lessons are NOT seeded here anymore. The previous version re-created a
// fixed set of demo lessons on every deploy, which fought the admin's
// curated catalog (deletions came back after the next deploy). Lessons
// are now an admin-managed surface end-to-end. Tenant scaffolding stays
// seeded — a fresh deploy with no tenant has no allowed domain, so no
// employee could sign in to bootstrap things.
//
